...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($maxTransactionIdResult === false) {
        ...
        $newTransactionId2= $newTransactionId + 1;
    } else {
        $newTransactionId = 1;
    }
    ...
    if ($conn->query($deleteSql) === TRUE) {
        ...
        if ($propertyType == 'Sale') {
            $transactionType = 'buysale';
        } elseif ($propertyType == 'Rent') {
            $transactionType = 'buyrent';
        } else {
            // Handle 'Both' type separately
            $transactionTypeSale = 'buysale';
            $transactionTypeRent = 'buyrent';

            // Insert transactions for both sale and rent
            $insertTransactionSqlSale = "INSERT INTO transactions (transaction_id, user_id, transaction_type, property_id, agent_id) 
                VALUES ('$newTransactionId', '$userID', '$transactionTypeSale', '$propertyId', '$agentID')";

            $insertTransactionSqlRent = "INSERT INTO transactions (transaction_id, user_id, transaction_type, property_id, agent_id) 
                VALUES ('$newTransactionId2', '$userID', '$transactionTypeRent', '$propertyId', '$agentID')";

            if ($conn->query($insertTransactionSqlSale) === TRUE && $conn->query($insertTransactionSqlRent) === TRUE) {
                echo "Transactions records added successfully for sale and rent.";
            } else {
                echo "Error adding transaction records: " . $conn->error;
            }

            exit; // Exit to avoid additional processing for 'Both' type
        }

        // Insert transaction record
        $insertTransactionSql = "INSERT INTO transactions (transaction_id, user_id, transaction_type, property_id, agent_id) 
            VALUES ('$newTransactionId', '$userID', '$transactionType', '$propertyId', '$agentID')";
